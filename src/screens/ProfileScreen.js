...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    const fetchDbUser = async () => {
      ...
      const userId = route.params?.id || authUser.attributes.sub;

      if (!userId) {
        return;
      }

      const isMe = userId === authUser.attributes.sub;

      const dbUser = await DataStore.query(User, userId);
      if (!dbUser) {
        if (isMe) {
          //the auth user does not have an account navigate to update account to create one in the db
